...
import {getCategories,updateArticle} from '../actions/articleActions'
...
function EditArticle({show,closeModal,body}) {
    ...
    useEffect(()=>{
        if(body.tags!==undefined){
            setTags(pluck(body.tags))
        }
        setTitle(body.title)
        setDescription(body.description)
        setInfo(body.content)
        setPosition(body.position)
        setKeyword(body.keywoard)
    },[body])
    const myfun=()=>{
    ...
    const onSubmit=()=>{
        const data={information:info,title,creation_date:date,description,position,keywoard:keyword,category_id:1,picture:file,tags:JSON.stringify(tags)};
        dispatch(updateArticle(data,body.id))
        close();
    }
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                    <View style={styles.modalContent}>
                        ...
                        <View style={globalStyles.container}>
                            <ScrollView>
                                <TextInput style={globalStyles.input} placeholder='title' value={title} onChangeText={(val)=>setTitle(val)}  />
                                <TextInput style={globalStyles.input} placeholder='description' value={description} onChangeText={(val)=>setDescription(val)} />
                                <TextInput style={globalStyles.input} placeholder='info' value={info} onChangeText={(val)=>setInfo(val)} />
                                <TextInput style={globalStyles.input} placeholder='position' value={position} onChangeText={(val)=>setPosition(val)} />
                                <TextInput style={globalStyles.input} placeholder='keyword' value={keyword} onChangeText={(val)=>setKeyword(val)} />
                                <Picker
